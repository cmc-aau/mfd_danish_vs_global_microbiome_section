...
fl.asv <- data.table::fread("data/2025-03-31_MFD_16S_UMI_OTU.csv", sep = ",", header = TRUE) %>%
  rename(OTU = V1) %>%
...
## Select samples based on minimum read count (<4,000 reads)
...
data.table::fwrite(metadata.subset, sep = "\t", row.names = FALSE, col.names = TRUE, quote = FALSE,
                   paste0("output/", format(Sys.time(), "%Y-%m-%d"), "_MFD_FL16S_metadata_subset.tsv"))
data.table::fwrite(metadata.filt, sep = "\t", row.names = FALSE, col.names = TRUE, quote = FALSE,
                   paste0("output/", format(Sys.time(), "%Y-%m-%d"), "_MFD_FL16S_metadata_filtered.tsv"))
...
data.table::fwrite(fl.asv.subset, sep = ",", row.names = FALSE, col.names = TRUE, quote = FALSE,
                   paste0("output/", format(Sys.time(), "%Y-%m-%d"), "_MFD_FL16S_OTU_subset.csv"))
data.table::fwrite(fl.asv.filt, sep = ",", row.names = FALSE, col.names = TRUE, quote = FALSE,
                   paste0("output/", format(Sys.time(), "%Y-%m-%d"), "_MFD_FL16S_OTU_filtered.csv"))
...
data.table::fwrite(fl.asv.subset.ra, sep = ",", row.names = FALSE, col.names = TRUE, quote = FALSE,
                   paste0("output/", format(Sys.time(), "%Y-%m-%d"), "_MFD_FL16S_OTU_subset_ra.csv"))
data.table::fwrite(fl.asv.filt.ra, sep = ",", row.names = FALSE, col.names = TRUE, quote = FALSE,
                   paste0("output/", format(Sys.time(), "%Y-%m-%d"), "_MFD_FL16S_OTU_filtered_ra.csv"))
...
data.table::fwrite(as.data.frame(object.iNEXT.total$AsyEst), sep = "\t", row.names = FALSE, col.names = TRUE, quote = FALSE,
                   paste0("output/", format(Sys.time(), "%Y-%m-%d"), "_MFD_FL16S_iNEXT_richness_total.tsv"))
...
data.table::fwrite(richness.measure, sep = "\t", row.names = FALSE, col.names = TRUE, quote = FALSE,
                   paste0("output/", format(Sys.time(), "%Y-%m-%d"), "_MFD_FL16S_iNEXT_richness_MFDO1.tsv"))
...
data.table::fwrite(richness.measure.ra, sep = "\t", row.names = FALSE, col.names = TRUE, quote = FALSE,
                   paste0("output/", format(Sys.time(), "%Y-%m-%d"), "_MFD_FL16S_iNEXT_richness_MFDO1_ra.tsv"))
...
plot.iNEXT <- ggiNEXT(object.iNEXT, type = 1, color.var = "Assemblage", facet.var = ) +
...
### save plots
png(file = 'output/MFD_FL16S_estimated_richness.png',
    width = 1900,
    height = 1000) 
plot.iNEXT
dev.off()

pdf(file = 'output/MFD_FL16_estimated_richness.pdf',
    width = 19,
    height = 12)
plot.iNEXT
dev.off()

tiff(file = 'output/MFD_FL16S_estimated_richness.tiff',
     width = 1900,
     height = 1200)
plot.iNEXT
dev.off()

ggsave("output/MFD_FL16S_estimated_richness.svg", plot = plot.iNEXT, width = 19, height = 12, units = "in", dpi = "retina")

### Save image
save.image(paste0('output/', format(Sys.time(), "%Y-%m-%d"), "_MFD_FL16S_estimated_richness.RData"))
