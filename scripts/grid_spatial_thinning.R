...
data <- data.table::fread('data/2024-03-07_arcbac-rarefaction-rel.csv')
...
data.grid <- readxl::read_excel('data/downloads/2024-02-13_mfd_db.xlsx') %>%
  filter(fieldsample_barcode %in% metadata.filt$fieldsample_barcode,
         !coords_reliable == "No",
         !is.na(latitude),
         !is.na(mfd_hab1)) %>%
...
data.grid.10km <- data.grid %>%
  filter(fieldsample_barcode %in% data.10km) %>%
  mutate(complex = str_c(mfd_sampletype, mfd_areatype, mfd_hab1, sep = ", ")) %>%
  group_by(complex) %>%
  mutate(complex_size = n()) %>%
  mutate(label = str_c(complex, ", n = ", complex_size, sep = "")) %>%
...
data.grid.1km <- data.grid %>%
  filter(fieldsample_barcode %in% data.1km) %>%
  mutate(across(mfd_hab1, ~str_replace(., "Sclerophyllous scrub", "Temperate heath and scrub")),
         across(mfd_hab2, ~str_replace(., "Scrub", "Sclerophyllous scrub"))) %>%
  mutate(complex = str_c(mfd_sampletype, mfd_areatype, mfd_hab1, sep = ", ")) %>%
  group_by(complex) %>%
...
data.table::fwrite(data.grid.10km, "output/2024-05-10_samples-grid-10km.csv")
data.table::fwrite(data.grid.1km, "output/2024-05-10_samples-grid-1km.csv")
...
## Save plot
pdf("output/samples_MFDO1_groups-grid.pdf")
plot.grid
dev.off()
...
## 10 km
pdf("output/map_MFDO1_groups-10km-grid.pdf")
for (i in 1:length(var.list.10km)) {
  print(plot.10km[[i]])
}
dev.off()
...
## 1 km
pdf("output/map_MFDO1_groups-1km-grid.pdf")
for (i in 1:length(var.list.1km)) {
  print(plot.1km[[i]])
}
dev.off()
